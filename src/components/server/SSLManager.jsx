  ...
  return (
    <div className="space-y-8">
      <div className={`glass-card ${mobileLayout ? 'p-4' : 'p-10'}`}>
        <div className={`flex items-center justify-between ${mobileLayout ? 'mb-6' : 'mb-12'}`}>
          <div className={`flex items-center ${mobileLayout ? 'gap-3' : 'gap-6'}`}>
            <div className={`${mobileLayout ? 'p-3' : 'p-4'} bg-[var(--accent-mint)] rounded-2xl shadow-lg shadow-mint-500/20 text-black`}>
              <ShieldCheck className={mobileLayout ? 'w-5 h-5' : 'w-6 h-6'} />
            </div>
            <div>
              <h3 className={`${mobileLayout ? 'text-base' : 'text-xl'} font-black uppercase tracking-tight font-display`}>SSL Gateway</h3>
              <p className="text-[10px] font-bold text-[var(--text-secondary)] uppercase tracking-widest mt-1">Let's Encrypt certificates</p>
            </div>
          </div>
          <div className="flex items-center gap-3">
            ...
            {isAdmin && (
              <button 
                ...
                className={`${mobileLayout ? 'px-4 py-2.5' : 'px-8 py-3'} bg-[var(--accent-mint)] text-black rounded-xl text-[10px] font-black uppercase tracking-widest hover:scale-105 transition-all active:scale-95`}
              >
                {mobileLayout ? 'Provision' : 'Provision SSL'}
              </button>
            )}
          </div>
        </div>
        ...
        {loading ? (
          ...
          <div className={`grid gap-4 ${mobileLayout ? 'grid-cols-1' : 'grid-cols-1 md:grid-cols-2 gap-8'}`}>
            {certs.map((cert) => {
              ...
              return (
                <div key={cert.name} className={`${mobileLayout ? 'p-4' : 'p-8'} bg-white/5 border border-white/5 rounded-3xl hover:bg-white/10 transition-all group`}>
                  <div className={`flex items-center justify-between ${mobileLayout ? 'mb-4' : 'mb-8'}`}>
                    <div className="flex items-center gap-3 min-w-0">
                      <div className={`p-2 rounded-xl shrink-0 ${isExpired ? 'bg-red-500/20 text-red-500' : 'accent-bg-green text-black'}`}>
                        <ShieldCheck className="w-4 h-4" />
                      </div>
                      <span className="text-sm font-black text-white uppercase tracking-tight truncate">{cert.name}</span>
                    </div>
                    {isAdmin && (
                      <button
                        onClick={() => handleRenew(cert.name)}
                        disabled={actionLoading === `renew-${cert.name}`}
                        className="shrink-0 px-3 py-1.5 rounded-lg bg-white/10 text-white text-[10px] font-black uppercase tracking-widest hover:bg-[var(--accent-mint)] hover:text-black transition-all disabled:opacity-50"
                      >
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    ...
                    <div className="space-y-1">
                      ...
                      <p className={`text-xs font-bold ${isExpired ? 'text-red-500' : daysLeft < 30 ? 'text-amber-500' : 'text-[var(--accent-mint)]'}`}>
                        {isExpired ? 'EXPIRED' : daysLeft < 30 ? `${daysLeft}D LEFT` : 'SECURED'}
                      </p>
                    </div>
                  </div>
                </div>
      ...
      {showIssueModal && (
        <div className={`fixed inset-0 z-[200] flex items-center justify-center ${mobileLayout ? 'p-4' : 'p-8'}`}>
           <div className="absolute inset-0 bg-black/90 backdrop-blur-md" onClick={() => !actionLoading && setShowIssueModal(false)} />
           <div className={`glass-card w-full max-w-lg relative z-10 ${mobileLayout ? 'p-5 max-h-[85vh] overflow-y-auto custom-scrollbar' : 'p-10'}`}>
              <div className="flex items-center justify-between mb-8">
                 <h3 className="text-xl font-black uppercase tracking-tight font-display">Deploy New SSL</h3>
              ...
              <div className="space-y-6">
                ...
                <div className="space-y-3">
                  ...
                  <select
                    ...
                    className="w-full px-4 py-4 bg-black/40 border border-[var(--border-color)] rounded-2xl text-sm text-white focus:border-[var(--accent-violet)] outline-none transition-all font-bold appearance-none uppercase"
                  >
                ...
                <div className="space-y-3">
                  ...
                  <input
                    ...
                    className="w-full px-4 py-4 bg-black/40 border border-[var(--border-color)] rounded-2xl text-sm text-white focus:border-[var(--accent-violet)] outline-none transition-all font-bold"
                  />
                </div>

                <div className="pt-4 flex gap-3">
                  <button onClick={() => setShowIssueModal(false)} className="flex-1 px-6 py-4 rounded-xl bg-white/5 text-white text-[10px] font-black uppercase tracking-widest hover:bg-white/10 transition-all">
                    Cancel
                  </button>
                  <button
                    onClick={handleIssue}
                    disabled={actionLoading === 'issue' || !issueForm.domain}
                    className="flex-1 px-6 py-4 rounded-xl bg-[var(--accent-mint)] text-black text-[10px] font-black uppercase tracking-widest hover:scale-105 transition-all active:scale-95 shadow-lg shadow-mint-500/20"
                  >
